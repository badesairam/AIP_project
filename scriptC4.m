function C4frames = scriptC4(maskedframes,R1,R2,W,niter)
    sz = 120*120;
    W4 = zeros(4*sz,4*sz);
    for j=1:4
    ...
    W4(sz+1:2*sz,2*sz+1:3*sz) = W;
    W4(sz+1:2*sz,3*sz+1:4*sz) = W;
    W4(2*sz+1:3*sz,3*sz+1:4*sz) = W;
    intialize = zeros(4*sz,1);
    C4frames = zeros(120,120,20);
    for i = 1:17
        ...
        C4frames(:,:,i) = reshape(W*x(1:sz),120,120);
    end
    C4frames(:,:,18) = reshape(W*(x(1:sz)+x(sz:2*sz)),120,120);
    C4frames(:,:,19) = reshape(W*(x(1:sz)+x(2*sz:3*sz)),120,120);
    C4frames(:,:,20) = reshape(W*(x(1:sz)+x(3*sz:4*sz)),120,120);
end
